...
BigInt* subtractBigInts(BigInt *b1, BigInt *b2)
{
	...
	if(b1->signedBit != b2->signedBit)//Bcoz since thir original signs are opposite and we have to subtract them,we basically have to add the magnitudes and then give appropriate sign
	{
		if(b2->signedBit==1)
		{
			b2->signedBit=0;
...
BigInt* multBigInts(BigInt *b1, BigInt *b2)
{
	...
	if(mult->length==0)	//if input is 0, product is 0
	{
		mult->signedBit=0; //if mult by negative number, - sign would have come,so it must be resetted to +
		mult->length=1;
	}
